...
==================
*/
int CM_SwitchMap( int world, qboolean client ) {
	int i, prev = cw;
...
cmodel_t	*CM_ClipHandleToModel( clipHandle_t handle ) {
	...
	for(w = 0; w < numWorlds; w++) {
		if((handle - total) >= 0 && (handle - total) < cm[w].numSubModels) {
			if(handle == 0) return &cm[cw].cmodels[0];
			if(w != cw) break; // return &cm[cw].cmodels[handle];
	...
	if ( handle < MAX_SUBMODELS ) {
		Com_Error( ERR_DROP, "CM_ClipHandleToModel: bad handle (world %i/%i/%i) %i < %i < %i", 
			cw, w, numWorlds, handle, cm[cw].numSubModels, MAX_SUBMODELS );
	}
