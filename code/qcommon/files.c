...
extern void Sys_FS_Startup(const char *gameName, qboolean closemfp);
extern void Sys_FS_Shutdown(const char *gameName, qboolean closemfp);
...
static void FS_Startup( const char *gameName )
{
	...
	Sys_FS_Startup(gameName, qfalse);
}
...
#if EMSCRIPTEN
	// FIXME: it would be really nice to use long_jmp and async callbacks hand in hand, emscripten -asyncify doesn't work for fastcomp apparently. I don't know C well enough - brian
	if(!isRestart) {
...
void FS_Restart( int checksumFeed ) {
	...
	FS_Startup(com_basegame->string);
#else
	Sys_FS_Shutdown(com_basegame->string, qfalse);
	// startup automatically called by shutdown in javascript via qfalse
}
