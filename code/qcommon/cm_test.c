...
void CM_StoreLeafs( leafList_t *ll, int nodenum ) {
	...
	if ( cm[cw].leafs[ leafNum ].cluster != -1 ) {
		ll->lastLeaf = leafNum;
	}
...
void CM_StoreBrushes( leafList_t *ll, int nodenum ) {
	...
	leaf = &cm[cw].leafs[leafnum];

	for ( k = 0 ; k < leaf->numLeafBrushes ; k++ ) {
		brushnum = cm[cw].leafbrushes[leaf->firstLeafBrush+k];
		b = &cm[cw].brushes[brushnum];
		if ( b->checkcount == cm[cw].checkcount ) {
			continue;	// already checked this brush in another leaf
		}
		b->checkcount = cm[cw].checkcount;
		for ( i = 0 ; i < 3 ; i++ ) {
	...
	for ( k = 0 ; k < leaf->numLeafSurfaces ; k++ ) {
		patch = cm[cw].surfaces[ cm[cw].leafsurfaces[ leaf->firstleafsurface + k ] ];
		if ( !patch ) {
			continue;
		}
	}
...
byte	*CM_ClusterPVS (int cluster) {
	if (cluster < 0 || cluster >= cm[cw].numClusters || !cm[cw].vised ) {
		return cm[cw].visibility;
	}

	return cm[cw].visibility + cluster * cm[cw].clusterBytes;
}
...
void CM_FloodArea_r( int areaNum, int floodnum) {
	...
	area = &cm[cw].areas[ areaNum ];

	if ( area->floodvalid == cm[cw].floodvalid ) {
	...
	area->floodvalid = cm[cw].floodvalid;
	con = cm[cw].areaPortals + areaNum * cm[cw].numAreas;
	for ( i=0 ; i < cm[cw].numAreas  ; i++ ) {
		if ( con[i] > 0 ) {
...
void	CM_AdjustAreaPortalState( int area1, int area2, qboolean open ) {
	...
	if ( area1 >= cm[cw].numAreas || area2 >= cm[cw].numAreas ) {
		Com_Error (ERR_DROP, "CM_ChangeAreaPortalState: bad area number");
	}

	if ( open ) {
		cm[cw].areaPortals[ area1 * cm[cw].numAreas + area2 ]++;
		cm[cw].areaPortals[ area2 * cm[cw].numAreas + area1 ]++;
	} else {
		cm[cw].areaPortals[ area1 * cm[cw].numAreas + area2 ]--;
		cm[cw].areaPortals[ area2 * cm[cw].numAreas + area1 ]--;
		if ( cm[cw].areaPortals[ area2 * cm[cw].numAreas + area1 ] < 0 ) {
...
qboolean	CM_AreasConnected( int area1, int area2 ) {
	...
	if (area1 >= cm[cw].numAreas || area2 >= cm[cw].numAreas) {
		Com_Error (ERR_DROP, "area >= cm[cw].numAreas");
	}

	if (cm[cw].areas[area1].floodnum == cm[cw].areas[area2].floodnum) {
		return qtrue;
	}
	return qfalse;
}
...
int CM_WriteAreaBits (byte *buffer, int area)
{
	...
	{
		floodnum = cm[cw].areas[area].floodnum;
		for (i=0 ; i<cm[cw].numAreas ; i++)
		{
			if (cm[cw].areas[i].floodnum == floodnum || area == -1)
				buffer[i>>3] |= 1<<(i&7);
		}
	}
