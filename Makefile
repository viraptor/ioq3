...
ifeq ($(PLATFORM),darwin)
  ...
  ifeq ($(USE_LOCAL_HEADERS),1)
    ...
    ifneq ($(findstring $(ARCH),ppc ppc64),)
      CLIENT_CFLAGS += -I$(SDLHDIR)/include-macppc
    else
...
# debug optimize flags: --closure 0 --minify 0 -g -g4
  OPTIMIZEVM += -O3 --closure 0 --minify 0 -g4
  OPTIMIZE = $(OPTIMIZEVM)
  ...
  CLIENT_LDFLAGS += --js-library $(LIBSYSCOMMON) \
    ...
    -s EXPORTED_FUNCTIONS="['_main', '_malloc', '_free', '_atof', '_strncpy', '_memset', '_memcpy', '_Com_Printf', '_FS_Startup_After_Async', '_Sys_Quit', '_Com_Error', '_Com_GetCDN', '_Com_GetManifest', '_Z_Malloc', '_Z_Free', '_S_Malloc', '_Cvar_Set', '_Cvar_VariableString', '_VM_GetCurrent', '_VM_SetCurrent', '_Sys_GLimpInit', '_Cbuf_ExecuteText', '_Cbuf_Execute', '_Cbuf_AddText', '_Com_ExecuteCfg']" \
  ...
  SERVER_LDFLAGS += --js-library $(LIBSYSCOMMON) \
    ...
    -s EXPORTED_FUNCTIONS="['_main', '_malloc', '_free', '_atof', '_strncpy', '_memset', '_memcpy', '_Com_Printf', '_FS_Startup_After_Async', '_Sys_Quit', '_Com_Error', '_Com_GetCDN', '_Com_GetManifest', '_Z_Malloc', '_Z_Free', '_S_Malloc', '_Cvar_Set', '_Cvar_VariableString', '_CON_SetIsTTY', '_VM_GetCurrent', '_VM_SetCurrent']" \
...
BASE_CFLAGS += -Werror-implicit-function-declaration -Werror
